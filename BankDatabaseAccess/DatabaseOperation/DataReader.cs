...
namespace BankDatabaseAccess.DatabaseOperation
{
    public class DataReader
    {
        ...
        /// <returns>This returns a datatable of the given object from the database</returns>
        public DataTable GetSingleData(EntityModel.PersonModel personModel, bool customer, bool employee)
        {
            var tableName = Table(customer, employee);
            if (tableName == null)
                throw new System.ArgumentException("Either customer or employee must be true");
                
            query = @"SELECT * 
                        FROM dbo.[dbo."+ tableName +"] " +
                        "WHERE Username =  '"+ personModel.Username +"'";
            return DataTable();
        }
        /// <summary>
        /// This method reads the whole data from a table except Passwords
        /// </summary>
